...
function Cases({ t }: { t: typeof T.en }) {
  const [ref, vis] = useReveal();
  return (
    <section ref={ref} className="relative z-10 pt-28 md:pt-40 pb-10 md:pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className={`flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-4 transition-all duration-700 ${vis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <div>
            ...
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-extrabold text-white mt-4 leading-[1.02] tracking-tight">{t.work.casesTitle}</h2>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-4 md:gap-5 max-w-2xl mx-auto">
          {t.work.cases.map((p, i) => (
            <a
              key={p.slug}
              href={`/work/${p.slug}`}
            ...
            >
              <div className="aspect-[16/10] relative overflow-hidden" style={{ background: `linear-gradient(150deg, #0A0A0C 0%, ${p.accent}22 100%)` }}>
                <img
                  src={p.image}
                  alt={p.title}
                  className="absolute inset-0 w-full h-full object-cover object-top scale-[1.02] transition-transform duration-700 group-hover:scale-105"
                  onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = "none"; }}
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/25 to-black/5" />
                <span className="absolute top-5 right-5 md:right-7 text-[2.5rem] md:text-[5rem] font-extrabold text-white/[0.04] group-hover:text-white/[0.08] transition-all duration-700 leading-none">0{i + 1}</span>
                <div className="absolute inset-0 flex flex-col justify-end p-5 sm:p-8 lg:p-10">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-[10px] tracking-[0.25em] uppercase font-medium" style={{ color: p.accent }}>{p.cat}</span>
                    <span className="text-[10px] opacity-70" style={{ color: p.accent }}>{p.year}</span>
                  </div>
...
function Work({ t }: { t: typeof T.en }) {
  ...
  return (
    <section ref={ref} className="relative z-10 pt-10 md:pt-16 pb-20 md:pb-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        <div className={`flex flex-col md:flex-row md:items-end justify-between mb-10 md:mb-16 gap-4 transition-all duration-700 ${vis ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          <div>
